...
function buildStaticPostureMetrics(keypoints: CameraKeypoint[]): Record<string, number | string | boolean | null> {
  ...
  const pelvicTilt =
    hasHips ? Math.abs(leftHip.y - rightHip.y) : 0;
  ...
  // 1. Calculate physical structure width/height for normalization (distance invariance)
  const shoulderWidth =
    leftShoulder && rightShoulder
      ? Math.sqrt(Math.pow(leftShoulder.x - rightShoulder.x, 2) + Math.pow(leftShoulder.y - rightShoulder.y, 2))
      : 0;

  const hipWidth =
    hasHips
      ? Math.sqrt(Math.pow(leftHip.x - rightHip.x, 2) + Math.pow(leftHip.y - rightHip.y, 2))
      : 0;

  const torsoHeight =
    leftShoulder && rightShoulder && hasHips
      ? Math.abs((leftShoulder.y + rightShoulder.y) / 2 - (leftHip.y + rightHip.y) / 2)
      : 0;

  const shoulderHeightDiffRatio = shoulderWidth > 0.01 ? shoulderHeightDiff / shoulderWidth : shoulderHeightDiff;
  const pelvisHeightDiffRatio = hasHips && hipWidth > 0.01 ? pelvicTilt / hipWidth : null;
  const trunkLateralShiftRatio = hasHips && torsoHeight > 0.01 ? trunkShift / torsoHeight : null;

  // 2. Suspect asymmetry direction
  let suspectedDirection: "left" | "right" | "unclear" = "unclear";
  if (leftShoulder && rightShoulder && Math.abs(leftShoulder.y - rightShoulder.y) > 0.003) {
    // Coordinate y=0 is at the top of the screen, so larger y means lower height on screen
    suspectedDirection = leftShoulder.y > rightShoulder.y ? "right" : "left";
  }

  return {
    shoulder_height_diff_ratio: Math.round(shoulderHeightDiffRatio * 1000) / 1000,
    pelvis_height_diff_ratio: pelvisHeightDiffRatio !== null ? Math.round(pelvisHeightDiffRatio * 1000) / 1000 : null,
    trunk_lateral_shift_ratio: trunkLateralShiftRatio !== null ? Math.round(trunkLateralShiftRatio * 1000) / 1000 : null,
    suspected_direction: suspectedDirection,
  };
}

function buildAdamsMetrics(keypoints: CameraKeypoint[]): Record<string, number | string | boolean | null> {
  const leftShoulder = findKeypoint(keypoints, "left_shoulder");
  const rightShoulder = findKeypoint(keypoints, "right_shoulder");
  const leftHip = findKeypoint(keypoints, "left_hip");
  const rightHip = findKeypoint(keypoints, "right_hip");
  ...
  const thoracicAsymmetryRatio = shoulderWidth > 0.01 ? thoracicAsymmetry / shoulderWidth : thoracicAsymmetry;
  const lumbarAsymmetryRatio = hipWidth > 0.01 ? lumbarAsymmetry / hipWidth : lumbarAsymmetry;

  let suspectedSide: "left" | "right" | "unclear" = "unclear";
  if (leftShoulder && rightShoulder && Math.abs(leftShoulder.y - rightShoulder.y) > 0.003) {
    suspectedSide = leftShoulder.y > rightShoulder.y ? "right" : "left";
  }

  return {
    forward_bend_completed: true,
    stable_hold_seconds: 2.0,
    thoracic_asymmetry: Math.round(thoracicAsymmetryRatio * 1000) / 1000,
    lumbar_asymmetry: Math.round(lumbarAsymmetryRatio * 1000) / 1000,
    suspected_side: suspectedSide,
  };
}

function buildSquatMetrics(liveMetrics: LiveSquatMetricsResponse): Record<string, number | string | boolean | null> {
  return {
...
export function ProtocolCapturePage() {
  ...
  const [frameHistory, setFrameHistory] = useState<CameraKeypoint[][]>([]);

  const handleCameraAnalysis = useCallback(
    (analysis: CameraFrameAnalysisResponse) => {
      const kps = analysis.keypoints ?? [];
      setLatestKeypoints(kps);
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      let perFrameMetrics: Record<string, number | string | boolean | null>[] | undefined = undefined;

      if (protocolType === "squat" && liveMetrics) {
        metrics = buildSquatMetrics(liveMetrics);
      } else if (protocolType === "adams_forward_bend") {
        const framesToUse = frameHistory.length > 0 ? frameHistory : [latestKeypoints];
        perFrameMetrics = framesToUse.map(buildAdamsMetrics);
        metrics = buildAdamsMetrics(latestKeypoints);
      } else {
        const framesToUse = frameHistory.length > 0 ? frameHistory : [latestKeypoints];
        perFrameMetrics = framesToUse.map(buildStaticPostureMetrics);
