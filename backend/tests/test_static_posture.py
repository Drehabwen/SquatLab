"""Tests for spine-focused static posture analysis: normative data, severity grading, PSI."""
...
def test_get_reference_returns_age_sex_matched_norm():
    ...
    # New angle-based norms: mean=1.4, sd=0.9 for male 10-13
    assert norm.mean == 1.4
    assert norm.sd == 0.9


def test_get_reference_falls_back_when_no_demographics():
    norm = get_reference("hip", age=None, sex="unknown")
    # Fallback norms: mean=1.1, sd=0.7
    assert norm.mean == 1.1
    assert norm.sd == 0.7


def test_compute_z_score_near_population_mean():
    z = compute_z_score("shoulder", 1.4, age=12, sex="male")
    assert z == pytest.approx(0.0, abs=0.01)


def test_compute_z_score_two_sd_above_mean():
    z = compute_z_score("shoulder", 3.2, age=12, sex="male")
    # mean=1.4, sd=0.9 → z = (3.2-1.4)/0.9 = 2.0
    assert z == pytest.approx(2.0, abs=0.05)


def test_compute_z_score_floored_at_zero():
    z = compute_z_score("shoulder", 0.5, age=12, sex="male")
    assert z == 0.0


def test_compute_z_score_without_demographics_uses_fallback():
    z = compute_z_score("trunk", 3.2, age=None, sex="unknown")
    # fallback mean=0.3, sd=0.22 → z = (3.2-0.3)/0.22 ≈ 13.2
    assert z > 10.0


def test_classify_severity_levels():
    # Shoulder thresholds: mild=2.0, moderate=5.0, severe=10.0
    assert classify_by_threshold("shoulder", 0.5) == "none"   # < 1.0
    assert classify_by_threshold("shoulder", 1.0) == "mild"   # < 2.0
    assert classify_by_threshold("shoulder", 1.5) == "mild"   # < 2.0
    assert classify_by_threshold("shoulder", 2.0) == "mild"   # < 5.0
    assert classify_by_threshold("shoulder", 5.0) == "moderate"  # < 10.0
    assert classify_by_threshold("shoulder", 7.0) == "moderate"
    assert classify_by_threshold("shoulder", 10.0) == "severe"
    assert classify_by_threshold("shoulder", 15.0) == "severe"


def test_classify_by_threshold_fallback():
    # Hip thresholds: mild=2.0, moderate=5.0, severe=10.0
    assert classify_by_threshold("hip", 0.0) == "none"
    assert classify_by_threshold("hip", 1.0) == "mild"
    assert classify_by_threshold("hip", 1.9) == "mild"
    assert classify_by_threshold("hip", 2.0) == "mild"
    assert classify_by_threshold("hip", 4.9) == "mild"
    assert classify_by_threshold("hip", 5.0) == "moderate"
    assert classify_by_threshold("hip", 9.9) == "moderate"
    assert classify_by_threshold("hip", 10.0) == "severe"


def test_classify_by_threshold_different_axes():
    # Trunk thresholds: mild=0.5, moderate=1.5, severe=3.0 (cm)
    assert classify_by_threshold("trunk", 0.2) == "none"
    assert classify_by_threshold("trunk", 0.4) == "mild"
    assert classify_by_threshold("trunk", 0.6) == "mild"
    assert classify_by_threshold("trunk", 2.0) == "moderate"
...
def test_psi_moderate_asymmetry():
    # z=1.0 → axis score = 100 * exp(-0.35) ≈ 70.5
    # shoulder 40% + hip 35% + trunk 25% = 100%
    # score ≈ 70.5 * 1.0 = 70.5
...
def test_psi_severe_asymmetry():
    # z=4.0 → axis score = 100 * exp(-1.4) ≈ 24.7
    score = compute_psi(shoulder_z=4.0, hip_z=4.0, trunk_z=4.0)
    assert score < 30.0


def test_psi_mixed_severity():
    # shoulder_z=0→100, hip_z=2→~50, trunk_z=1→~70
    # weighted: 100*0.40 + 50*0.35 + 70*0.25 = 40 + 17.5 + 17.5 = 75
...
def test_derive_static_posture_all_normal(service, normal_metrics):
    findings, flags, recs, enriched = service._derive_static_posture_v2(
        normal_metrics, subject_age=12, subject_sex="female",
    )
    assert "未见明显脊柱不对称" in findings[0]
    assert flags == []
    assert enriched is not None
    assert enriched["severity_grades"]["shoulder"] == "none"
    assert enriched["severity_grades"]["hip"] == "none"
    assert enriched["severity_grades"]["trunk"] == "none"
...
def test_derive_static_posture_moderate_shoulder():
    ...
    metrics = {
        "shoulder_angle_deg": 3.5,  # 3.5 degrees > mild threshold (2.0)
        "pelvic_tilt_deg": 1.0,
        "trunk_shift_cm": 0.3,
...
def test_derive_static_posture_without_demographics_falls_back(service):
    metrics = {
        "shoulder_angle_deg": 2.5,  # 2.5 degrees > mild threshold (2.0)
        "pelvic_tilt_deg": 1.5,
        "trunk_shift_cm": 0.4,
        "suspected_direction": "left",
    }
    findings, flags, recs, enriched = service._derive_static_posture_v2(metrics)
    assert enriched is not None
...
def test_derive_static_posture_old_field_names(service):
    ...
    findings, flags, recs, enriched = service._derive_static_posture_v2(metrics)
    assert enriched is not None
    assert "severity_grades" in enriched
...
def test_derive_static_posture_recommendations_vary_by_psi():
    service = ScreeningAnalysisService()
    # Severe case with large angle deviations
    severe = {
        "shoulder_angle_deg": 8.0,
        "pelvic_tilt_deg": 7.0,
        "trunk_shift_cm": 2.0,
        "suspected_direction": "right",
    }
    _, _, recs, _ = service._derive_static_posture_v2(severe, subject_age=10, subject_sex="male")
    assert len(recs) >= 2
    assert any("脊柱" in r for r in recs)
    ...
    _, _, recs, _ = service._derive_static_posture_v2(normal, subject_age=10, subject_sex="female")
...
def test_fuse_multi_frame_metrics():
    from app.features.screening.normative import fuse_multi_frame_metrics

    frames = [
        {"shoulder_height_diff_ratio": 0.02, "pelvis_height_diff_ratio": 0.01},
        {"shoulder_height_diff_ratio": 0.04, "pelvis_height_diff_ratio": 0.015},
        {"shoulder_height_diff_ratio": 0.03, "pelvis_height_diff_ratio": 0.012},
    ]
    fused, stability, is_unstable = fuse_multi_frame_metrics(frames)

    assert fused["shoulder_height_diff_ratio"] == pytest.approx(0.03)
    assert fused["pelvis_height_diff_ratio"] == pytest.approx(0.012)
    assert is_unstable is False


def test_fuse_multi_frame_metrics_unstable():
    from app.features.screening.normative import fuse_multi_frame_metrics

    frames = [
        {"shoulder_height_diff_ratio": 0.01},
        {"shoulder_height_diff_ratio": 0.08},
        {"shoulder_height_diff_ratio": 0.02},
        {"shoulder_height_diff_ratio": 0.015},
        {"shoulder_height_diff_ratio": 0.012},
    ]
    fused, stability, is_unstable = fuse_multi_frame_metrics(frames)
    # With outlier rejection, 0.08 should be rejected
    assert fused["shoulder_height_diff_ratio"] < 0.05
